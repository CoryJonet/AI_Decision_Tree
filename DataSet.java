...
public class DataSet {
	...
	for (int i = 0; i < labels.size(); i++) {
	    if (splitline[0].equals(labels.get(i))) {
		instance.label = new Integer(i);
		break;
	    }
	}
	...
	for (int i = 1; i < splitline.length; i++) {
	    List<String> values = attributeValues.get(attributes.get(i - 1));
	    // find the index of the value
	    for (int j = 0; j < values.size(); j++) {
		if (splitline[i].equals(values.get(j))) {
		    instance.addAttribute(j);
		    break;
		}
		if (j == values.size() - 1) {
    ...
    /**
     * Verifies that two DataSets use the same values for
     * labels and attributes as wells as the same ordering.
